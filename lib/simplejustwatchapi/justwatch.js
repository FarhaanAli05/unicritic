 ...
 * @returns {Promise<any[]>}
 */
export async function search(title, country = 'US', language = 'en', count = 4, best_only = true) {
  const request = prepare_search_request(title, country, language, count, best_only);
  ...
  if (!resp.ok) {
    ...
    const err = new Error(`JustWatch API returned HTTP ${resp.status}: ${text}`);
 ...
 */
export async function details(node_id, country = 'US', language = 'en', best_only = true) {
  const request = prepare_details_request(node_id, country, language, best_only);
  const resp = await fetch(_GRAPHQL_API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(request),
 ...
 */
export async function offersForCountries(node_id, countries, language = 'en', best_only = true) {
  const countryArray = Array.isArray(countries) ? countries : Array.from(countries || []);
  if (!countryArray || countryArray.length === 0) return {};
  const request = prepare_offers_for_countries_request(node_id, countryArray, language, best_only);
  const resp = await fetch(_GRAPHQL_API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(request),
