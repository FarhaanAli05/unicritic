 ...
 * @returns {Object}
 */
export function prepare_search_request(
  title,
  country,
  language,
  count,
...
export function parse_search_response(json) {
  if (
    !json ||
    !json.data ||
    !json.data.popularTitles ||
    !Array.isArray(json.data.popularTitles.edges)
  ) {
    return [];
  }
 ...
 * @returns {Object}
 */
export function prepare_offers_for_countries_request(
  node_id,
  countries,
  language,
  best_only,
...
function _assert_country_code_is_valid(code) {
  if (!code || String(code).length !== 2)
    throw new Error(
      `Invalid country code: ${code}, code must be 2 characters long`,
    );
}

function _prepare_offers_for_countries_entry(countries) {
  const offer_requests = countries.map((country_code) =>
    _GRAPHQL_COUNTRY_OFFERS_ENTRY.replace(
      /{country_code}/g,
      country_code.toUpperCase(),
    ),
  );
  const main_body = _GRAPHQL_OFFERS_BY_COUNTRY_QUERY.replace(
    "{country_entries}",
    offer_requests.join("\n"),
  );
...
function _parse_entry(json) {
  ...
  const backdrops = (content.backdrops || [])
    .filter(Boolean)
    .map((bd) => _IMAGES_URL + bd.backdropUrl);
...
function _parse_scores(json) {
  ...
  const tomatometer =
    json.tomatoMeter != null ? Number(json.tomatoMeter) : null;
...
function _parse_streaming_charts(json) {
  const edges =
    json && json.streamingCharts && Array.isArray(json.streamingCharts.edges)
      ? json.streamingCharts.edges
      : null;
  if (!edges || edges.length === 0) return null;
  const info =
    edges[0] && edges[0].streamingChartInfo
      ? edges[0].streamingChartInfo
      : null;
  if (!info) return null;
...
function _parse_offer(json) {
  ...
  const last_change_retail_price_value =
    json.lastChangeRetailPriceValue ?? null;
